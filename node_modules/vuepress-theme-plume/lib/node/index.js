import { deepMerge, hasOwn, isArray, isEmptyObject, isFunction, isNumber, isPlainObject, isString, omit, sleep, toArray, uniq } from "@pengzhanbo/utils";
...
import { Logger, addViteConfig, addViteOptimizeDepsExclude, addViteOptimizeDepsInclude, addViteSsrNoExternal, encodeData, ensureEndingSlash, ensureLeadingSlash, entries, fromEntries, getFullLocaleConfig, isArray as isArray$1, isBoolean, isFunction as isFunction$1, isLinkAbsolute, isLinkHttp, isLinkWithProtocol, isPlainObject as isPlainObject$1, removeEndingSlash, removeLeadingSlash } from "@vuepress/helper";
...
function withBase(path$2 = "", base = "/") {
	...
	if (path$2.startsWith(base)) return normalizePath$1(path$2);
	return normalizePath$1(`${base}${path$2}`);
}
...
function withBlogAndNotesHaveBeenDelete(options) {
	if ("blog" in options && (!options.collections || options.collections.length === 0)) logger.warn(t$5("blog"));
...
async function resolveEncrypt(encrypt) {
	...
	if (!isEmptyObject(encryptRules)) for (const key of keys) {
		...
		rules$1[String(index)] = (await pMap(toArray(encryptRules[key]).filter(isStringLike), (item) => genEncrypt(item))).join(separator);
	}
	return [
...
function llmsPlugin$1(app, userOptions) {
	if (!app.env.isBuild) return [];
	const { llmsTxtTemplateGetter, ...userLLMsTxt } = isPlainObject$2(userOptions) ? userOptions : {};
	function tocGetter(llmPages, llmState) {
		...
		const generateTOCLink$1 = (path$2) => {
			const filepath = path$2.endsWith("/") ? `${path$2}README.md` : path$2.endsWith(".md") ? path$2 : `${path$2 || "README"}.md`;
			const link = path$2.endsWith("/") ? `${path$2}index.html` : `${path$2}.html`;
...
function normalizePath(prefix, path$2 = "") {
	if (path$2.startsWith("/")) return path$2;
	return `${ensureEndingSlash$1(prefix)}${path$2}`;
}
...
function getEffectsByFrontmatter(app) {
	...
	for (const page of app.pages) {
		...
		if (!(fm.home || fm.pageLayout === "home") || config?.length === 0) continue;
		for (const item of config) if (item.type === "hero") {
			const effect = item.effect;
			if (effect) if (allEffects.includes(effect)) effects.push(effect);
